...
static const unsigned int numMutex = 1001;

HTTPCache::HTTPCache(): mutexes(numMutex) {
  ...
  //  for(auto int i = 0; i < numMutex; ++i)
  //    this->mutexes.push_back(
}
...
void HTTPCache::handleMutex(const HTTPRequest & request, const unsigned int lockOrUnlock){
  size_t hashValue = this->hashRequest(request);
  
  if(lockOrUnlock == LOCK)
    this->mutexes[hashValue%numMutex].lock();

  if(lockOrUnlock == UNLOCK)
    this->mutexes[hashValue%numMutex].unlock();
}
