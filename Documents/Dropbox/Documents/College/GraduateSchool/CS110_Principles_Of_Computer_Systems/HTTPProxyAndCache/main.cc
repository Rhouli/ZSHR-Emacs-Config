...
int main(int argc, char *argv[]) {
  ...
  try {
    ...
    log("Listening for all incoming traffic on port " + to_string(proxy.getPortNumber()) + ".");
    if (proxy.isUsingProxy()) {
      log("Requests will be directed toward another proxy at " +
	  proxy.getProxyServer() + ":" + to_string(proxy.getProxyPortNumber()) + ".");
    }
    while (true) {
	proxy.acceptAndProxyRequest();
    }
  } catch (const HTTPProxyException& hpe) {
    log("Fatal Error: " + string(hpe.what()));
    log("Exiting..... ");
