...
HTTPProxy::HTTPProxy(int argc, char *argv[]) throw (HTTPProxyException): 
  portNumber(computeDefaultPortForUser()), 
  usingProxy(false), 
  usingSpecificProxyPortNumber(false),
  proxyPortNumber(computeDefaultPortForUser()), 
...
void HTTPProxy::configureFromArgumentList(int argc, char *argv[]) throw (HTTPProxyException) {
  ...
  if(this->usingProxy)
    this->scheduler.setRequestProxy(this->proxyServer, this->proxyPortNumber);
}
